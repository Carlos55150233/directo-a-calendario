...
def onmessage(update,bot:ObigramClient):
    try:
        ...
        if username == tl_admin_user or user_info:  # validate user
            ...
            #mensagito ="El bot esta en MANTENIMIENTO\n"
            intento_msg = "💢El usuario @"+username+ " ha intentando usar el bot sin permiso💢"
            bot.sendMessage(update.message.chat.id,mensaje)
            #bot.sendMessage(update.message.chat.id,mensagito)
        ...
        if '/addadmin' in msgText:
            ...
            if isadmin:
                try:
                    ...
                    jdb.save()
                    msg = '😃Genial @'+user+' ahora tiene acceso al bot y es Admin👍'
                    bot.sendMessage(update.message.chat.id,msg)
                except:
                    bot.sendMessage(update.message.chat.id,'❌Error en el comando /adduser username❌')
            else:
...
def main():
    ...
    bot_token = '5356497692:AAF6gMlv0B-SCZLojwQbSh9Wo28l5x0Id0Y'
